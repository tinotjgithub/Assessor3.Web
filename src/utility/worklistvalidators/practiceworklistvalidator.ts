 ...
 */
class PracticeWorklistValidator implements workListValidatorSchema {
    ...
    public submitButtonValidate(response: ResponseBase) {
        ...
        switch (worklistStore.instance.getResponseMode) {
            case enums.ResponseMode.open:
                this.responseStatuses = this.openResponseValidation(response as PracticeOpenResponse);
                break;
        }
     ...
     * @param response
     */
    private openResponseValidation(response: PracticeOpenResponse) {
...
export = PracticeWorklistValidator;
